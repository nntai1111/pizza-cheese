...
@Component({
  selector: 'app-kitchen-order-detail',
  imports: [RouterLink, DatePipe],
  templateUrl: './kitchen-order-detail.component.html',
  styleUrls: [
    '../../customer/order-detail/order-detail.component.scss',
    './kitchen-order-detail.component.scss',
  ],
})
export class KitchenOrderDetailComponent {
  ...
  startPreparing(): void {
    ...
    this.kitchenService.startPreparing(current.id).subscribe({
      next: (order) => {
      ...
      error: (err: HttpErrorResponse) => {
        this.actionLoading.set(false);
        this.errorMessage.set(getHttpErrorMessage(err, 'Không thể nhận đơn.'));
      },
    });
  }
  ...
  markReady(): void {
    ...
    this.kitchenService.markReady(current.id).subscribe({
      next: (order) => {
        this.order.set(order);
        this.actionLoading.set(false);
      },
      error: (err: HttpErrorResponse) => {
        this.actionLoading.set(false);
        this.errorMessage.set(getHttpErrorMessage(err, 'Không thể hoàn thành đơn.'));
      },
    });
  }
  ...
  private loadOrder(orderId: string): void {
    this.kitchenService.getOrder(orderId).subscribe({
      next: (order) => {
        this.order.set(order);
        this.loading.set(false);
      },
